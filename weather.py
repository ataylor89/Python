...
def main():
    ...
    weather_report = get_weather_report(latitude, longitude)
...
# tempData = resp.json()['properties']['temperature']
# temps = []

# if tempData['uom'] == 'wmoUnit:degC':
#     for reading in tempData['values']:
#         temp = {}
#         datetime = reading['validTime'].split("/PT")[0]
#         datetime = parser.parse(datetime).astimezone()
#         temp['date'] = datetime.strftime("%Y-%m-%d")
#         if temp['date'] != today:
#             break
#         temp['time'] = datetime.strftime("%H:%M:%S %Z")
#         temp['period'] = reading['validTime'].split("/PT")[1]
#         temp['celsius'] = reading['value']
#         temp['fahrenheit'] = celsius_to_fahrenheit(temp['celsius'])
#         temps.append(temp)

# print("Today's date: %s" %today)
# for temp in temps:
#     cels = format(temp['celsius'], '.1f')
#     fahr = format(temp['fahrenheit'], '.1f') 
#     time = temp['time']
#     period = temp['period']
#     print("Temperature: %s\u00b0F (%s\u00b0C) at %s for %s" %(fahr, cels, time, period))
